...
async def get_order(message:types.Message):
    ...
    with open('photo1.png', 'rb') as photo1:
        await message.answer_photo(photo1, caption='1. 3 стрипса (Оригинальные / Острые)\n\nНежное куриное филе мы панируем вручную и готовим в ресторане по уникальному рецепту Полковника Сандерса. Совершенный вкус!\n\n Цена: 654')
...
async def add_order(message:types.Message):
    cursor = connect.cursor()
    if message.text == '1':
        cursor.execute(f"INSERT INTO orders VALUES('{message.from_user.id}', '3 стрипса (Оригинальные / Острые)', '' , '{time.ctime()}');")
    elif message.text == '2':
